...
(defn heading [text]
  [:div
   [:h1
    {:class "text-4xl font-semibold text-gray-800 capitalize lg:text-5xl"}
    text]
   [:div
    {:class "mt-2"}
    [:span {:class "inline-block w-40 h-1 bg-gray-800 rounded-full"}]
    [:span
     {:class "inline-block w-3 h-1 ml-1 bg-gray-800 rounded-full"}]
    [:span
     {:class "inline-block w-1 h-1 ml-1 bg-gray-800 rounded-full"}]]])
